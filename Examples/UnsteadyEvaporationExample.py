...
output_folder_name = "Examples/UnsteadyEvaporationExample/"
...
x_dim = L[1]-L[0]
y_dim = H[1]-H[0]
y_interface_pos = H[0] + 1.0*y_dim/3
domain_dim = 2

# Jumps conditions parameters
m_dot = 1.0 # using m_dot != 0 leads to a non-solvable system...
deltaTangentStress = 0.0#0.1
deltaTangentVel = 0.0#-0.1
delta_p = -0.2
deltaTemp = 0.5
heatSource = 1.0
...
# Temperatures
T_0_user = 0.0*sp.cos(0.75*sp.pi*sym_variables[1]) + 2.0
...
# T_1_user = 0.1*sym_variables[1]*T_coeff[0]+ T_coeff[1] + 0.0*sym_variables[1] #+ 2.25
T_1 = UserDefinedManufSol(T_coeff, sym_variables, T_1_user)

# Pressures
p_0_user = 0.0*sp.cos(0.75*sp.pi*sym_variables[1]) + 2.45
p_0 = UserDefinedManufSol([], sym_variables, p_0_user)

if pressureNormalStressDecoupled and splitting_2:
    p_coeff = sp.symbols('bP_1, bP_2')
    p_1_user = 0.0*sp.cos(5*sp.pi*sym_variables[1]/(4*y_dim)) + p_coeff[0] + p_coeff[1]
else:
    p_coeff = [sp.symbols('bP_1')]
    # p_1_user = p_coeff[0]*sp.cos(5*sp.pi*sym_variables[1]/(4*y_dim)) + 2.25
    p_1_user = 0.0*sp.cos(5*sp.pi*sym_variables[1]/(4*y_dim)) + p_coeff[0]
...
# Vertical velocities
v_0_user = 0.0*sym_variables[1]-0.5#1.0*sp.cos(0.75*sp.pi*sym_variables[1]/y_dim) + 2.45
...
vel_interface = 0.5 - m_dot/rho_0 # by definition of the interface mass flux: m_dot = rho_0 * (vel_n_0 - vel_n_gamma) = rho_1 * (vel_n_1 - vel_n_gamma)
vel_interface *= -1.0 # WARNING: to get the y-component of the velocity and not the normal projection along the interface vel_n_0
print(">>>>>>>>>>>> VEL INTERFACE = %2.12f"%vel_interface)
...
u_gamma_n = vel_interface * n_gamma[1] #u_n_ave - m_dot * overRho_ave 
...
if pressureHeatFluxDecoupled:
    ...
    if m_dot != 0 and splitting_2:
        ...
        iso0_jump.addDirichletBCAndSubsituteBoundaryEq(FluidSolutionTags.TOTALENERGY, [jump_convective_total_energy], coord_to_subsituted)
else:
...
my_problem = GeneralProblemHandler([eqs_0, eqs_1], [iso0_jump], False, False)
...
for time_plot in time_plot_vec:
    ...
    quantities_plot = [p_plot, rho_plot, T_plot, vel_n_plot, vel_t_plot, u_plot, v_plot]#
