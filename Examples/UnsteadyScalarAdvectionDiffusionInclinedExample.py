...
#* -------------------------------------------------------------------------- *#
new_spatial_sym_variables = sp.symbols('x, y')
angle_rotation = -15
my_new_problem = my_problem.copyAndChangeCoordinatesSystem(list(new_spatial_sym_variables), CoordinatesSystemType.CARTESIAN, None, [[2,angle_rotation]])
params_sol_new = my_new_problem.getParametricSolCoeff()
print(" ")
print("======== Printing solutions vectors of both phases INCLINED ========")
print("*** Phase 0 ***")
for i in my_new_problem.getThisPhysicalModel(name_model_0).getSolVectorFromTags([],[], params_sol_new):
    print(i)
print("*** Phase 1 ***")
...
iso0_plot_geo = my_new_problem.getThisBoundary("iso0").getBoundaryGeometry()
iso0_plot_geo.setBoundaryEquation(iso0_plot_geo.getBoundaryEquation().subs({sym_variables[-1]: time_plot}))
iso0_plot_geo.setSymVariables(new_spatial_sym_variables)
plot_area = PlotOver2DAreaWithStraightBoundaries(new_spatial_sym_variables, [iso0_plot_geo], [pt_1, pt_2, pt_3, pt_4])

nb_lines_plot = 4
coords_plot = np.linspace(-y_dim, y_dim, nb_lines_plot)
lines_plot = []
for i in range(nb_lines_plot):
    lines_plot.append(BoundaryGeometryFromEquation(new_spatial_sym_variables, [0.0, 1.0, coords_plot[i]], "plot_line_"+str(i)))
...
my_new_problem.plotQuantitiesAlongOneDimLinesOverThisArea(quantities_plot, lines_plot, plot_area, side_interface_plot, 30, 0)

my_new_problem.plotQuantitiesOverThisTwoDimArea(quantities_plot, plot_area, side_interface_plot, 30)
